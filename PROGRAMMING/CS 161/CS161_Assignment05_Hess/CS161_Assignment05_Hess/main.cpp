...
int main()
{
	...
	type = "width";
	width = GetValue(type);						// Get width using user defined function GetValue

	type = "height";
	height = GetValue(type);						// Get height using user defined function GetValue
...
int GetValue(string type)
{
	...
	while (retest == 0)										// Loop A (Get End Value and Test)
	{
		...
		if ((value > 0) && (value < 25))					// Loop A4 (Checks for valid numeric value)											
		{
			retest = 1;
		}
		...
		{
			if ((value <= 0) && (!(error == 3)))			// Loop A5a (Error # to small)
				cout << "You have entered an invalid " << type << ". Please enter an integer greater than " << "0" << ".";

			if ((value >= 25) && (!(error == 3)))			// Loop A5b (Error # to large)
				cout << "You have entered an invalid " << type << ". Please enter an integer less than " << "25" << ".";
...
//after running, noticed that there is a missing period at the end of the try again statement. 
//also, it allows zeros in the beginning of a number
